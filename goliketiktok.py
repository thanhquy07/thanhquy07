...
def changetoken(red,green,white):
  if os.path.exists("cache_golike_auth.txt"):
    text=f'''{green}Bạn Muốn Dùng Authorization Cũ Hay Đổi Authorization Mới
{red}[{white}1{red}] Dùng Authorization Cũ
{red}[{white}2{red}] Đổi Authorization Mới'''
    pr3(text)
...
{red}            └───────────────────────┘
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
{red}Chú Ý {cyan}Chú Ý ⚠️!!!!

 ~[+]{green}Tiền Sau Khi Làm Sẽ Được Cộng Sau Vài Phút
 
 ~[+]{blue}Kiểm Tra Không Thấy Lên Xu Không Phải Do Tool Lỗi
 ...
 while True :
  while True :
    if not os.path.exists("cache_golike_auth.txt"):
      auth=str(input(f'~[+]{red}Nhập Authorization:{green} '))
      headers ={
    'Authorization'	:auth,
...
# Chuỗi JSON đầu vào
        ...
        pr(text)
        print('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
        text=f'~[+]{red}Select {green}Acc Làm Nhiệm Vụ '
        pr(text)
        ...
        text=f'~[+]{red}Lỗi! Authorization Không Chính Xác>>{green}Vui Lòng Nhập Lại'
        continue 
    else:
...
def get_id_from_nickname_number(ranmau,check,red,blue,green,yellow,cyan,magenta,orange,xanhnhat,xduong,pink):
  while True :
    user_input=input(f'~[+]{random.choice(ranmau)}>{random.choice(ranmau)}>{random.choice(ranmau)}> {green}Chọn Acc Để Chạy Job:{green} ')
    try:
      n = int(user_input)
      if 'data' in check and len(check['data']) >= n:
          idtiktok = check['data'][n-1]['id']
          if idtiktok :
              text=f"{red}Ib Của NickName Là: {green}{idtiktok}"
              pr(text)
              print('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
              return idtiktok 
          else:
    ...
    while True:
      while True:
        try:
            ...
            print(f"{red}Có Lỗi Gì Đó, Đang Nhận Lại Job...")
            sleep(2)
            pass
      try:
        link=a['data']['link']
        ...
        for k in range(delay,-1,-1):
            ...
            print(f'{green}SUCCESS:{red}[{job_success}/{startmaxjob}]{random.choice(ranmau)}Loading{random.choice(ranmau)}>>{yellow}Nhiệm Vụ Mới Sau{random.choice(ranmau)}>>{random.choice(ranmau)}[{k}s]',end='\r')
            sleep(1)
        print(f'{green}Đang Kiểm Tra Job...',end='\r')
        headers = {
            'authorization': auth,
        't':	'VFZSWk5VOUVVVEJQUkZGNFRXYzlQUT09',
        'User-Agent':"Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_3_5 like Mac OS X; en-us) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8L1 Safari/6533.18.5"
        ...
        while True:
            try:
                ...
                print(f'{red}Lỗi, Đang Load Lại...',end="\r")
                sleep(2)
                pass
        if g['status']==200:
            job_success+=1
            print(f'{green}SUCCESS:{red}[{job_success}]{cyan}[{time()}]|{random.choice(ranmau)}SUCCESS|{green}Follow|+{g["data"]["prices"]}')
            startmaxjob+=1
            jobloi=0
            ...
            print(f'{green}Đang Kiểm Tra Job Lại...',end="\r")
            sleep(2)
            while True:
                try:
                    g = requests.post('https://gateway.golike.net/api/advertising/publishers/tiktok/complete-jobs',headers=headers,json=json_data).json()
                    ...
                    print(f'{red}Đang Nhận Lại Tiền Job...',end="\r")
                    sleep(2)
            if g['status']==200:
                job_success+=1
                print(f'{green}SUCCESS:{red}[{job_success}]{cyan}[{time()}]|{random.choice(ranmau)}SUCCESS|{green}Follow|+{g["data"]["prices"]}')
                ...
                print(f'{red}Đang Bỏ Qua Job...',end='\r')
                headers = {
                    'authorization': auth,
                    't':	'VFZSWk5VOUVVVEJQUkZGNFRXYzlQUT09',
                    'User-Agent':"Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_3_5 like Mac OS X; en-us) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8L1 Safari/6533.18.5"
                ...
                json_data = {
                    'description': 'Hoàn Thành Thất Bại',
                ...
                if startmaxjob == maxjob+1:
                    ...
                    select=input(f'{red}Lỗi Nhiều ,Bạn Có Muốn Đổi Acc?(y/n):')
                    if select.lower() == 'n':
                        pass
                    else:
                        nicknames = [item['nickname'] for item in check['data'] if 'nickname' in item]
          ...
          print(f'{red}Đang Nhận Lại Job...',end='\r')
          sleep(2)
    ...
    while True:
      ...
      try:
        ...
        if 'video' in link:
            ...
            for k in range(delay,-1,-1):
                mau=random.choice(ranmau)
                print(f'{green}SUCCESS:{red}[{job_success}]{random.choice(ranmau)}Loading{random.choice(ranmau)}>>{yellow}Nhiệm Vụ Mới Sau{random.choice(ranmau)}>>{random.choice(ranmau)}[{k}s]',end='\r')
                sleep(1)
            print(f'{green}Đang Kiểm Tra Job...',end='\r')
            headers = {
                'authorization': auth,
            ...
            if g['status']==200:
                ...
                print(f'{green}Đang Kiểm Tra Lại Job...',end="\r")
